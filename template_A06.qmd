...
[https://github.com/ConnorSchuster/Schuster_Connor_201_AO6](https://github.com/ConnorSchuster/Schuster_Connor_201_AO6)
...
```{r}
lebron_points_index <- player_info[[2]][["points"]]
print(lebron_points_index)

lebron_points_name <- player_info[["LJ"]][["points"]]
print(lebron_points_name)
...
for (player_name in names(player_info)) {
  ...
  points_per_game_list[[player_name]] <- points_per_game
}
...
```{r}
points_rounded <- sapply(points_per_game_list, function(x) round(x, 2))
...
rounded_matrix <- apply(mat_rand, MARGIN = 1:2, FUN = round_num)
...
```{r}
avg_temp <- apply(max_temps[, 3:14], 2, mean, na.rm = TRUE)
...
```{r}
state_CO <- filter(max_temps, state == "CO")
state_OH <- filter(max_temps, state == "OH")
state_VA <- filter(max_temps, state == "VA")
```
...
```{r}
state_CO <- max_temps[max_temps[["state"]] == "CO", ]

median_temp_CO <- apply(state_CO[, 3:14], 1, median, na.rm = TRUE)

annual_median_temp <- data.frame(year = state_CO[["year"]], median_temp = median_temp_CO)

max_index <- which.max(annual_median_temp[["median_temp"]])
min_index <- which.min(annual_median_temp[["median_temp"]])

max_temp <- annual_median_temp[max_index, "median_temp"]
min_temp <- annual_median_temp[min_index, "median_temp"]
max_year <- annual_median_temp[max_index, "year"]
min_year <- annual_median_temp[min_index, "year"]
...
```{r}
sum_digits <- function(num2) {
  if (num2 < 10) {
    return(num2)
  } else {
    return((num2 %% 10) + sum_digits(num2 %/% 10))
  }
}

check_result <- sum_digits(935068)
print(check_result)
```
